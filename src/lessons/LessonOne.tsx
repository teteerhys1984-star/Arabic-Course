 ...
 *
 * All 19 required steps and their authoritative source text are preserved exactly;
 * only the delivery mechanism changed from one continuous document to a sequential flow.
 * Interactive state that must survive navigating away from a step and back (most
 * importantly the official end-of-lesson test) is lifted here, above LessonFlow, so it
 * is never reset by moving between steps.
 */
export function LessonOne({ onProgressChange, onFinish }: LessonOneProps) {
  const [openSigns, setOpenSigns] = useState<Record<number, boolean>>({})
  const [aynSelected, setAynSelected] = useState<'أ' | 'ن' | 'ي' | 'ت'>('أ')
  const [verbSelected, setVerbSelected] = useState<Record<string, string>>({})
  ...
  function chooseChallenge(option: string) {
    if (option === 'اسم') {
      setChallengeFeedback('أحسنت! مدرسة اسم.')
    } else {
  ...
  const steps: LessonStepDefinition[] = [
      ...
      render: () => (
        <>
          ...
          <p className="source-note">
            ويجب على الطالب أن يحفظ هذه الأقسام الثلاثة جيدًا، لأنها أساس معظم دروس النحو القادمة.
          </p>
        </>
      ),
    },
    {
      ...
      render: () => (
        <>
          ...
          <p className="source-note">
            <bdi>إنسان، حيوان، نبات، جماد، مكان، زمان، صفة</bdi>
          </p>
          <ul className="inline-pills">
            {['إنسان', 'حيوان', 'نبات', 'جماد', 'مكان', 'زمان', 'صفة'].map((item) => (
              <li key={item}>{item}</li>
            ))}
          </ul>
        ...
        <>
          ...
          <div className="sign-grid">
            {nounSigns.map((sign, index) => (
              <NounSignCard sign={sign} index={index} open={!!openSigns[index]} onToggle={() => toggleSign(index)} key={sign.title} />
            ))}
          </div>
          ...
          <div className="source-subsection" id="al-definition">
            ...
            <div className="attention-box">
              <strong>انتبه:</strong>
              <p>لا يمكننا القول:</p>
              <p className="bidi-line">
                <DirectionText direction="rtl">الـ يكتب</DirectionText> — <DirectionText direction="rtl">الـ ذهب</DirectionText>
              </p>
          ...
          <div className="source-subsection">
            ...
            <div className="tanween-grid" aria-label="أنواع التنوين">
              <span>
                تنوين الضم: <bdi>ــٌ</bdi>
              </span>
              <span>
                تنوين الفتح: <bdi>ــً</bdi>
              </span>
              <span>
                تنوين الكسر: <bdi>ــٍ</bdi>
              </span>
            </div>
          ...
          <div className="source-subsection">
            ...
            <div className="phrase-ribbon" dir="rtl">
              <bdi>من – إلى – عن – على – في – الباء – الكاف – اللام</bdi>
            </div>
          </div>
          ...
          />
          <div className="source-note source-note--important">
            <strong>ملاحظة مهمة للطالب:</strong> ليس من الضروري أن تجتمع كل العلامات في الكلمة. إذا ظهرت علامة واحدة
            واضحة، يمكننا غالبًا معرفة أن الكلمة اسم.
          </div>
        </>
      ),
    },
      ...
      render: () => (
        <div className="source-subsection">
          ...
          <ul className="quoted-list">
            ...
            <li>ذهبَ أبي إلى العمل.</li>
            <li>لعبَ الطفلُ بالكرة.</li>
            <li>نجحتْ الطالبةُ في الامتحان.</li>
          </ul>
          ...
          <p>
            الفعل <bdi>سافرَ</bdi> ماضٍ لأنه حدث وانتهى.
          </p>
        </div>
      ),
    },
    {
      ...
      render: () => (
        <div className="source-subsection">
          ...
          <p className="source-kicker">أمثلة</p>
          <ul className="quoted-list">
            <li>يكتبُ الطالبُ الدرسَ.</li>
            <li>يذهبُ أحمدُ إلى المدرسة.</li>
            <li>تلعبُ البنتُ بالكرة.</li>
            <li>ندرسُ اللغة العربية.</li>
          </ul>
          ...
          <div className="ayn-banner" dir="ltr" aria-label="أحرف المضارعة: أ ن ي ت">
            <bdi>أ – ن – ي – ت</bdi>
          </div>
          ...
          <div className="ayn-interaction">
            <AynActivity selected={aynSelected} onSelect={setAynSelected} />
          </div>
          <p className="source-kicker">أمثلة أحرف المضارعة</p>
          <div className="ayn-example-list" dir="ltr" aria-label="أمثلة أحرف المضارعة الأربعة">
            <bdi>أ = أكتب</bdi>
            <bdi>ن = نكتب</bdi>
            <bdi>ي = يكتب</bdi>
            <bdi>ت = تكتب</bdi>
          ...
          <p className="source-kicker">أمثلة</p>
          <DataTable
            caption="حرف المضارعة ومثاله"
            headers={['حرف المضارعة', 'مثال']}
            rows={[['أ', 'أدرس'], ['ن', 'نلعب'], ['ي', 'يقرأ'], ['ت', 'تكتب']]}
          />
          <div className="source-note source-note--important">
            <strong>ملاحظة مهمة:</strong> ليس كل كلمة تبدأ بهذه الأحرف فعلًا مضارعًا، لكن معظم الأفعال المضارعة تبدأ
            بأحدها.
          </div>
        </div>
      ),
    },
    {
      ...
      render: () => (
        <>
          ...
          <DataTable
            caption="مقارنة بين الماضي والمضارع والأمر"
            headers={['نوع الفعل', 'الزمن', 'مثال']}
            rows={[
              ['الماضي', 'حدث وانتهى', 'كتبَ'],
              ['المضارع', 'يحدث الآن أو لاحقًا', 'يكتبُ'],
              ['الأمر', 'طلب القيام بعمل', 'اكتبْ'],
            ]}
          />
          <VerbClassificationActivity selected={verbSelected} onSelect={(word, option) => setVerbSelected((current) => ({ ...current, [word]: option }))} />
        </>
      ),
    },
    {
      ...
      render: () => (
        <>
          ...
          <p>
            كلمة <bdi>في</bdi> وحدها لا تعطي معنى كاملًا.
          </p>
        ...
        <>
          <details className="worked-example" open>
            ...
            <p className="phrase-ribbon">
              <bdi>كتاب – ذهب – في – مدرسة – يقرأ – إلى</bdi>
            </p>
            <DataTable
              caption="حل المثال الأول"
              headers={['الكلمة', 'نوعها', 'السبب']}
              rows={[
    ...
    },
    {
      ...
      render: () => (
        <GrammarDetective
          ...
          onAnswer={(word, option) => setDetectiveAnswers((current) => ({ ...current, [word]: option }))}
        />
      ),
    },
    {
    ...
    },
    {
      ...
      render: () => (
        <>
          ...
          <div className="source-note source-note--large">
            <strong>القاعدة الجامعة:</strong> اسم – فعل – حرف.
          </div>
        </>
      ),
    },
    ...
    },
    {
      ...
      render: () => (
        <>
          <div className="hero__letters" aria-label="اسم فعل حرف" dir="rtl">
            {['اسم', 'فعل', 'حرف'].map((part, index) => (
          ...
          <p>
            تعرّفت في هذا الدرس على أقسام الكلام الثلاثة: <bdi>الاسم</bdi> و<bdi>الفعل</bdi> و<bdi>الحرف</bdi>، وعلامات
            كل قسم، وأتممت الاختبار الرسمي. راجع مساحة المعلم مع معلمك لمتابعة إجاباتك على الأسئلة المفتوحة.
          </p>
        </>
      ),
    },
...
function ArrowExamples({ rows }: { rows: string[][] }) {
  return (
    <div className="arrow-examples">
      {rows.map(([from, to]) => (
        <div className="arrow-example" key={`${from}-${to}`}>
          <bdi>{from}</bdi>
          <span aria-hidden="true">←</span>
          <bdi>{to}</bdi>
        </div>
...
function NounSignCard({
  sign,
  index,
  open,
  onToggle,
}: {
  sign: (typeof nounSigns)[number]
  index: number
  open: boolean
  onToggle: () => void
}) {
  return (
    <button className={`sign-card ${open ? 'sign-card--open' : ''}`} type="button" onClick={onToggle} aria-expanded={open}>
      <span className="sign-card__number">
        <bdi>{index + 1}</bdi>
      </span>
      <bdi className="sign-card__short">{sign.short}</bdi>
      <strong>{sign.title}</strong>
      <span className="sign-card__example">
        <bdi>{sign.example}</bdi>
      </span>
...
function AynActivity({
  selected,
  onSelect,
}: {
  selected: 'أ' | 'ن' | 'ي' | 'ت'
  onSelect: (letter: 'أ' | 'ن' | 'ي' | 'ت') => void
}) {
  const examples = { أ: 'أكتب', ن: 'نكتب', ي: 'يكتب', ت: 'تكتب' } as const
  return (
    <div className="ayn-activity" aria-labelledby="ayn-activity-title">
      <div className="activity-heading">
        ...
        <div>
          <p className="source-kicker">نشاط تفاعلي</p>
          <h4 id="ayn-activity-title">أحرف المضارعة</h4>
        </div>
      </div>
      <div className="ayn-buttons" dir="ltr">
        {(Object.keys(examples) as Array<keyof typeof examples>).map((letter) => (
          <button
            type="button"
            className={selected === letter ? 'ayn-button ayn-button--selected' : 'ayn-button'}
            key={letter}
      ...
      <p className="ayn-result">
        <bdi>{selected}</bdi> = <bdi>{examples[selected]}</bdi>
      </p>
    </div>
  )
}

function VerbClassificationActivity({
  selected,
  onSelect,
}: {
  selected: Record<string, string>
  onSelect: (word: string, option: string) => void
}) {
  const items = [
    ['كتبَ', 'ماضٍ'],
    ['يكتبُ', 'مضارع'],
  ...
  return (
    <div className="activity-panel" aria-labelledby="verb-activity-title">
      <div className="activity-heading">
        ...
        <div>
          <p className="source-kicker">تفاعل حول المصدر</p>
          <h4 id="verb-activity-title">صنّف الأفعال</h4>
        </div>
      </div>
      <div className="classification-list">
        {items.map(([word, answer]) => (
          <div className="classification-row" key={word}>
            <bdi>{word}</bdi>
            <div>
              {['ماضٍ', 'مضارع', 'أمر'].map((option) => (
                <button
                  type="button"
                  className={selected[word] === option ? 'mini-choice mini-choice--selected' : 'mini-choice'}
                  key={option}
                  onClick={() => onSelect(word, option)}
                >
                  {option}
                </button>
              ))}
            ...
            {selected[word] && (
              <span className={selected[word] === answer ? 'answer-mark answer-mark--good' : 'answer-mark'}>
                {selected[word] === answer ? 'صحيح' : `الصحيح: ${answer}`}
              </span>
            )}
          </div>
        ))}
...
function GrammarDetective({
  ...
  onAnswer: (word: string, option: string) => void
}) {
  return (
    <div className="activity-panel detective" aria-labelledby="detective-title">
      <div className="activity-heading">
        <span className="detective-icon" aria-hidden="true">
          🔍
        </span>
        <div>
          <p className="source-kicker">نشاط صفّي ممتع</p>
          <h3 id="detective-title">لعبة المحقق اللغوي</h3>
        </div>
      </div>
      <p>اكتب على بطاقات الكلمات التالية، ثم قسّم البطاقات إلى ثلاث مجموعات:</p>
      <p className="phrase-ribbon">
        <bdi>كتاب – يركض – في – شجرة – اكتب – إلى – مدرسة – لعب – يقرأ – يا</bdi>
      </p>
      <div className="detective-words">
        {detectiveWords.map(([word, answer]) => (
          <div className="detective-word" key={word}>
            <bdi>{word}</bdi>
            <div className="detective-options">
              {['اسم', 'فعل', 'حرف'].map((option) => (
                <button
                  type="button"
                  className={answers[word] === option ? 'mini-choice mini-choice--selected' : 'mini-choice'}
                  key={option}
                  onClick={() => onAnswer(word, option)}
                >
                  {option}
                </button>
              ))}
            ...
            {answers[word] && (
              <span className={answers[word] === answer ? 'answer-mark answer-mark--good' : 'answer-mark'}>
                {answers[word] === answer ? '✓' : `الصحيح: ${answer}`}
              </span>
            )}
          </div>
        ))}
      </div>
      <div className="detective-groups">
        <div>
          <strong>مجموعة الأسماء</strong>
          <span>كتاب – شجرة – مدرسة.</span>
        </div>
        <div>
          <strong>مجموعة الأفعال</strong>
          <span>يركض – اكتب – لعب – يقرأ.</span>
        </div>
        <div>
          <strong>مجموعة الحروف</strong>
          <span>في – إلى – يا.</span>
        </div>
      </div>
      <p className="source-note">
        التقسيم الصحيح: الأسماء: كتاب – شجرة – مدرسة. الأفعال: يركض – اكتب – لعب – يقرأ. الحروف: في – إلى – يا.
      </p>
    </div>
  )
}

function FiveSecondChallenge({
  started,
  feedback,
  onStart,
  onChoose,
  ...
  return (
    <div className="challenge-panel" aria-labelledby="challenge-title">
      <div className="activity-heading">
        ...
        <div>
          <p className="source-kicker">التحدي</p>
          <h3 id="challenge-title">تحدي 5 ثوانٍ</h3>
        </div>
      </div>
      ...
      {!started ? (
        ...
        <div className="challenge-question">
          <p>
            ما نوع كلمة <bdi>مدرسة</bdi>؟
          </p>
          {['اسم', 'فعل', 'حرف'].map((option) => (
            <button className="mini-choice" type="button" key={option} onClick={() => onChoose(option)}>
              {option}
            </button>
          ))}
          <p className="activity-feedback" role="status">
            {feedback || 'لديك 5 ثوانٍ.'}
          </p>
        </div>
      )}
    </div>
  ...
  return (
    <div className="teacher-material">
      ...
      <ol className="teacher-notes">
        <li>
          <strong>الخلط بين الاسم والفعل:</strong> قد يعتبر الطالب كلمة "لعب" اسمًا لأنها تدل على شيء يعرفه، لذلك يجب
          التركيز على ارتباط الفعل بالزمن.
        </li>
        <li>
          <strong>الخلط بين المضارع والأمر:</strong> يكتب = مضارع، اكتب = أمر. الفرق أن المضارع يخبر عن حدث، أما الأمر
          فيطلب القيام به.
        </li>
        <li>
          <strong>اعتبار كل كلمة تبدأ بحرف من أحرف المضارعة فعلًا مضارعًا:</strong> مثل كلمة "أحمد" ليست فعلًا، بل اسم
          علم.
        </li>
      </ol>
      ...
      <ul className="source-list">
        <li>تصنيف 15 كلمة من أصل 20 بشكل صحيح على الأقل.</li>
        <li>تحديد أنواع الأفعال الثلاثة دون مساعدة.</li>
        <li>استخراج أقسام الكلام من جمل قصيرة.</li>
        <li>ذكر علامات الاسم الأساسية.</li>
      </ul>
    </div>
  )
}
